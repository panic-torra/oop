...
BOOST_FIXTURE_TEST_SUITE(Car_class, CarFixture)
	...
	BOOST_AUTO_TEST_SUITE(if_car_engine_is_turned_off)
		...
		BOOST_AUTO_TEST_CASE(can_not_select_gear_when_engine_turned_off)
		{
	...
	BOOST_FIXTURE_TEST_SUITE(when_engine_turned_on, when_engine_turned_on_)
		...
		BOOST_AUTO_TEST_CASE(engine_cannot_be_turned_off_when_speed_is_not_equal_to_zero)
		{
			BOOST_CHECK(car.SetGear(Gear::FIRST));
		...
		BOOST_AUTO_TEST_CASE(engine_cannot_be_turned_off_twice)
		{
			BOOST_CHECK(car.TurnOffEngine());
			BOOST_CHECK(!car.IsEngineTurnOn());
			BOOST_CHECK(!car.TurnOffEngine());
		...
		BOOST_AUTO_TEST_CASE(gear_can_be_change_from_neutral)
		{
			BOOST_CHECK(car.SetGear(Gear::FIRST));
		...
		BOOST_AUTO_TEST_CASE(gear_cannot_be_changed_when_speed_is_out_of_range)
		{
			BOOST_CHECK(!car.SetGear(Gear::SECOND));
			BOOST_CHECK_EQUAL(car.GetGear(), static_cast<int>(Gear::NEUTRAL));
